...
批量采集微信公众号文章，通过日期筛选 + AI 主题分类，自动筛选出特定主题的文章。
...
输入起止日期后，脚本会：
1. 从 `data/articles.json` 中按日期区间筛选文章
2. 逐篇提取正文（前 800 字）
3. 调用 DeepSeek API 判断是否属于"外事"主题
4. 结果保存到 `data/filter_result.json`
...
└── filter_articles.py  # 步骤3: 日期筛选 + AI 分类
```
